/*
 * ARM Cortex-M7 Core Peripheral Access Layer
 * ARM Cortex-M7 核心外设访问层
 *
 * ============================================================================
 * File: core_cm7.h
 * Description: ARM Cortex-M7 core peripheral access layer definitions
 * 描述: ARM Cortex-M7 核心外设访问层定义
 *
 * This file defines all NVIC, SysTick, SCB, FPU, MPU, Cache functions for ARM Cortex-M7.
 * 本文件定义了 ARM Cortex-M7 的所有 NVIC、SysTick、SCB、FPU、MPU、缓存函数。
 *
 * Documentation:
 ...
 *
 * Note:
 *   This file depends on armv7m.h for architecture abstraction.
 *   本文件依赖于 armv7m.h 提供架构抽象层。
 *
 * ============================================================================
 ...
 * @brief 使能 I-Cache
 */
__STATIC_INLINE void __ICACHE_Enable(void) {
    uint32_t reg = SCB->CCR;
    if ((reg & SCB_CCR_IC_Msk) == 0UL) {
        __DMB();
 ...
 * @brief 禁用 I-Cache
 */
__STATIC_INLINE void __ICACHE_Disable(void) {
    __DMB();
    SCB->CCR &= ~(uint32_t)SCB_CCR_IC_Msk;
    __DSB();
    __ISB();
 ...
 * @brief 使能 D-Cache
 */
__STATIC_INLINE void __DCACHE_Enable(void) {
    uint32_t reg = SCB->CCR;
    if ((reg & SCB_CCR_DC_Msk) == 0UL) {
        __DMB();
 ...
 * @brief 禁用 D-Cache
 */
__STATIC_INLINE void __DCACHE_Disable(void) {
    __DMB();
    SCB->CCR &= ~(uint32_t)SCB_CCR_DC_Msk;
    __DSB();
    __ISB();
 ...
 * @brief 无效化 I-Cache
 */
__STATIC_INLINE void __ICACHE_Invalidate(void) {
    __DMB();
    __ISB();
}

/**
 * @brief 清理并无效化 D-Cache
 */
__STATIC_INLINE void __DCACHE_CleanInvalidate(void) {
    __DMB();
    __DSB();
}
