...
/*
 * ============================================================================
 * CMSIS Defines and Configuration
 * CMSIS 定义和配置
 *
 * This section provides Cortex-M33 specific configuration and identification.
 ...
 *   - 调试访问
 *
 * Reference: Arm® Cortex™-M33 Devices Generic User Guide
 *           Chapter 4 (The Cortex-M33 Peripherals)
 * Reference: ARM Cortex-M33 TRM Chapter 2 (Programmers Model)
 * 参考: Arm® Cortex™-M33 GUG 第 4 章 (The Cortex-M33 Peripherals)
 *       ARM Cortex-M33 TRM 第 2 章 (程序员模型)
 * ============================================================================
 */

/**
 * __CM33_REV - Cortex-M33 Core Revision
 * Cortex-M33 核心版本
 *
 * This define specifies the revision of the Cortex-M33 processor core.
 ...
 * 此定义指定 Cortex-M33 处理器核心的版本。
 * 版本号在 CPUID 寄存器的位 [23:20] 中编码。
 *
 * Reference: Arm® Cortex®-M33 TRM Section 1.9 (Product Revisions)
 *            Table 3-5 (CPUID bit assignments)
 * 参考: Arm® Cortex®-M33 TRM 第 1.9 节 (产品修订版本)
 *       表 3-5 (CPUID 位分配)
 */
#define __CM33_REV                 0x0001U

/**
 * __CM33_CPUID - Cortex-M33 CPUID Base Register Reset Value
 * Cortex-M33 CPUID 基址寄存器复位值
 *
 * The CPUID register identifies the processor type, version, and implementation details.
 * CPUID = 0x411FD210 for Cortex-M33 r1p0 indicates:
 *   - Processor: ARMv8-M Mainline (M=1)
 *   - Variant: D (Microcontroller profile)
 *   - Architecture: 8 (ARMv8-M)
 *   - Revision: 1p0
 *
 * CPUID 寄存器标识处理器类型、版本和实现细节。
 * 对于 Cortex-M33 r1p0，CPUID = 0x411FD210 表示:
 *   - 处理器: ARMv8-M Mainline (M=1)
 *   - 变体: D（微控制器配置文件）
 *   - 架构: 8 (ARMv8-M)
 *   - 版本: 1p0
 *
 * CPUID Register Bit Fields / CPUID 寄存器位字段:
 *   - [31:24] Implementer (0x41 = ARM)
 *   - [23:20] Variant (revision)
 *   - [19:16] Constant (0xF)
 *   - [15:4]  PartNo (0xD21 = Cortex-M33)
 *   - [3:0]   Revision
 *
 * Reference: Arm® Cortex®-M33 TRM Section 1.9 (Product Revisions)
 *            Section 3.5 (CPUID Base Register)
 *            Table 3-5 (CPUID bit assignments)
 * 参考: Arm® Cortex®-M33 TRM 第 1.9 节
 *       第 3.5 节 (CPUID 基址寄存器)
 *       表 3-5 (CPUID 位分配)
 */
 ...
 * 中断号定义
 *
 * This section defines the interrupt number enumeration for the Cortex-M33 processor.
 ...
 *
 *   - Exception numbers 16-255: External interrupts (外部中断)
 *     IRQ0-IRQ239 are available on Cortex-M33 (depends on implementation)
 *     IRQ0-IRQ239 在 Cortex-M33 上可用（取决于实现）
 *
 ...
 *
 * Reference: Arm® Cortex™-M33 Devices Generic User Guide
 *           Chapter 4.4 (Nested Vectored Interrupt Controller)
 *           Chapter 2.1.4 (Exception model)
 * Reference: Arm® Cortex®-M33 TRM
 *           Chapter 5 (Nested Vectored Interrupt Controller)
 * 参考: Arm® Cortex™-M33 GUG
 *       第 4.4 章（嵌套向量中断控制器）
 *       第 2.1.4 章（异常模型）
 *       ARM Cortex-M33 TRM
 *       第 5 章（嵌套向量中断控制器）
 ...
 * IRQn_Type - 中断号枚举
 *
 * This enumeration defines all exception and interrupt numbers for the Cortex-M33.
 ...
 *
 * Note: Not all Cortex-M33 implementations support all interrupts.
 *       The actual number of interrupts depends on the chip implementation.
 * 注意：并非所有 Cortex-M33 实现都支持所有中断。
 *       实际的中断数量取决于芯片实现。
 */
typedef enum {
    ...
    IRQ254_IRQn                 = 254  /*!< External Interrupt 254 / 外部中断 254 */
                                       /*!< Maximum IRQ number defined / 定义的最大 IRQ 号 */
} IRQn_Type;

/**
 * Total number of IRQs defined
 * 定义的 IRQ 总数
 *
 * This constant represents the maximum number of interrupt requests (IRQs)
 * supported by this CMSIS device header.
 *
 * Note: The actual hardware may support fewer or more interrupts.
 ...
 *
 * Reference: Arm® Cortex™-M33 Devices Generic User Guide
 *            Chapter 4.4 (NVIC) - "up to 256 interrupt priority levels"
 *            Chapter 2.1.4 (Exception model) - "IRQ0 to IRQ479"
 * 参考: Arm® Cortex™-M33 GUG
 *       第 4.4 章 (NVIC) - "最多 256 个中断优先级"
 *       第 2.1.4 章 (异常模型) - "IRQ0 到 IRQ479"
 */
