/*
 * ARM Cortex-M4 Core Peripheral Access Layer
 * ARM Cortex-M4 核心外设访问层
 *
 * ============================================================================
 * File: core_cm4.h
 * Description: ARM Cortex-M4 core peripheral access layer definitions
 * 描述: ARM Cortex-M4 核心外设访问层定义
 *
 * This file defines all NVIC, SysTick, SCB, FPU, MPU functions for ARM Cortex-M4.
 * 本文件定义了 ARM Cortex-M4 的所有 NVIC、SysTick、SCB、FPU、MPU 函数。
 *
 * Documentation:
...
/**
 * @brief 获取当前正在执行的中断
 * @brief Get the active status for a device specific interrupt
 */
...
__STATIC_INLINE uint32_t SCB_GetFPUType(void) {
    ...
    if      ((mvfr0 & (0xFUL << 0U)) == 0x020U) {
        return(1U);
    }
    else if ((mvfr0 & (0xFUL << 0U)) == 0x010U) {
        return(2U);
    }
    else {
        return(0U);
...
/**
 * @brief 获取 FPU CPACR 寄存器
 * @brief Get FPU CPACR
 */
__STATIC_INLINE uint32_t __FPU_GetCPACR(void) {
    return __FPU_ReadCPACR();
}

/**
 * @brief 设置 FPU CPACR 寄存器
 * @brief Set FPU CPACR
 */
__STATIC_INLINE void __FPU_SetCPACR(uint32_t cpacr) {
    __FPU_WriteCPACR(cpacr);
}
